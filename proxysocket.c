 ...
 void proxysocket_get_version (int* pmajor, int* pminor, int* pmicro)
{
 ...
 const char* proxysocket_get_version_string ()
{
  return PROXYSOCKET_VERSION_STRING;
}

const char* proxysocketconfig_get_type_name (int proxytype)
{
  switch (proxytype) {
...
int proxysocketconfig_add_proxy (proxysocketconfig proxy, int proxytype, const char* proxyhost, uint16_t proxyport, const char* proxyuser, const char* proxypass)
{
  ...
  proxy->proxyinfolist->proxyhost = (proxyhost ? _strdup(proxyhost) : NULL);
  proxy->proxyinfolist->proxyport = proxyport;
  proxy->proxyinfolist->proxyuser = (proxyuser ? _strdup(proxyuser) : NULL);
  proxy->proxyinfolist->proxypass = (proxypass ? _strdup(proxypass) : NULL);
...
char* proxysocketconfig_get_description_entry (proxysocketconfig proxy, struct proxyinfo_struct* proxyinfo, char* desc, int desclen)
{
 ...
 char* proxysocketconfig_get_description (proxysocketconfig proxy)
{
  char* result = NULL;
  int resultlen = 0;
